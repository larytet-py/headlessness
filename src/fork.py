...
def join_process(job_pid, timeout):
...
def kill_bill(job_pid, timeout, start_time):
    elapsed = time.time() - start_time
    if elapsed > timeout:
        http_statistics.deadline_reached += 1

    is_alive = job_is_alive(job_pid)
    if is_alive:
        http_statistics.still_is_alive += 1
        error = f"Deadline: {timeout}s reached, elapsed {elapsed}s, {job_pid}"
        logger.info(error)
    else:
        http_statistics.subprocess_ok += 1
    ...
    try:
        ...
        if is_alive:
            logger.error(f"Failed to kill {job_pid} {e}")
...
def call_timeout(timeout, func, data):
    ...
    async_read = AsyncRead(pipe_read);async_read.start()
